...
interface SocketAskLoopSmokeReport {
    finalTexts: string[];
...
async function main(): Promise<void> {
    const harness = new SocketAskLoopSmoke();
    const report = await harness.run();
    console.log(JSON.stringify(report, null, 2));
    if (!report.ok) process.exitCode = 1;
}

class SocketAskLoopSmoke {
    ...
    public async run(): Promise<SocketAskLoopSmokeReport> {
        this.root = await mkdtemp(join(tmpdir(), "flyflor-socket-ask-loop-smoke-"));
        try {
            this.config = await this.createConfig();
            const memory = new MemoryModule(this.config, this.events);
            await memory.warmup();
            const dispatch = new SocketAskLoopDispatch(memory, this.events);
            ...
            if (!serverUrl) throw new Error("Socket did not expose a server url");
