...
function buildFrames(examples: Record<string, JsonRecord>): FrameExample[] {
    return [
        clientFrame("ClientHello", "00 握手连接", ["Ack"], examples),
        clientFrame("Ping", "00 握手连接", ["Pong"], examples),
        serverFrame("ServerHello", "00 握手连接", examples),
        serverFrame("Ack", "00 握手连接", examples),
        serverFrame("Pong", "00 握手连接", examples),

        clientFrame("GatewayStatusGet", "01 控制面", ["GatewayStatusSnapshot"], examples),
        clientFrame("CapabilityCatalogGet", "01 控制面", ["CapabilityCatalogSnapshot"], examples),
        serverFrame("GatewayStatusSnapshot", "01 控制面", examples),
        serverFrame("CapabilityCatalogSnapshot", "01 控制面", examples),
        ...
        clientFrame("InvalidGatewayMessageSend", "02 实时对话", ["InvalidPayloadError"], examples),
        serverFrame("TurnDelta", "02 实时对话", examples),
        serverFrame("TurnFinal", "02 实时对话", examples),
        serverFrame("TurnFinalWithAsk", "02 实时对话", examples),
        serverFrame("TurnFinalWithPlanning", "02 实时对话", examples),
        serverFrame("TurnFinalWithExecutiveLoopPause", "02 实时对话", examples),
        serverFrame("TurnError", "02 实时对话", examples),
        serverFrame("InvalidPayloadError", "02 实时对话", examples),

        clientFrame("HistoryList", "03 TUI 只读查询", ["HistorySnapshot"], examples),
        clientFrame("HistoryDetailGet", "03 TUI 只读查询", ["HistoryDetailSnapshot"], examples),
        clientFrame("ScopeList", "03 TUI 只读查询", ["ScopeSnapshot"], examples),
        clientFrame("ScopeDetailGet", "03 TUI 只读查询", ["ScopeDetailSnapshot"], examples),
        clientFrame("ForkList", "03 TUI 只读查询", ["ForkListSnapshot"], examples),
        clientFrame("ForkDetailGet", "03 TUI 只读查询", ["ForkSnapshot"], examples),
        clientFrame("AskList", "03 TUI 只读查询", ["AskSnapshot"], examples),
        clientFrame("AskDetailGet", "03 TUI 只读查询", ["AskDetailSnapshot"], examples),
        clientFrame("BlackboardList", "03 TUI 只读查询", ["BlackboardSnapshot"], examples),
        clientFrame("BlackboardDetailGet", "03 TUI 只读查询", ["BlackboardDetailSnapshot"], examples),
        clientFrame("TaskList", "03 TUI 只读查询", ["TaskSnapshot"], examples),
        clientFrame("TaskDetailGet", "03 TUI 只读查询", ["TaskDetailSnapshot"], examples),
        clientFrame("ReplayList", "03 TUI 只读查询", ["ReplaySnapshot"], examples),
        clientFrame("ReplayDetailGet", "03 TUI 只读查询", ["ReplayDetailSnapshot"], examples),
        clientFrame("ThoughtDetailGet", "03 TUI 只读查询", ["ThoughtSnapshot"], examples),
        clientFrame("CrystalList", "03 TUI 只读查询", ["CrystalSnapshot"], examples),

        serverFrame("HistorySnapshot", "04 TUI 快照响应", examples),
        serverFrame("HistoryDetailSnapshot", "04 TUI 快照响应", examples),
        serverFrame("ScopeSnapshot", "04 TUI 快照响应", examples),
        serverFrame("ScopeDetailSnapshot", "04 TUI 快照响应", examples),
        serverFrame("ForkListSnapshot", "04 TUI 快照响应", examples),
        serverFrame("ForkSnapshot", "04 TUI 快照响应", examples),
        serverFrame("AskSnapshot", "04 TUI 快照响应", examples),
        serverFrame("AskDetailSnapshot", "04 TUI 快照响应", examples),
        serverFrame("BlackboardSnapshot", "04 TUI 快照响应", examples),
        serverFrame("BlackboardDetailSnapshot", "04 TUI 快照响应", examples),
        serverFrame("TaskSnapshot", "04 TUI 快照响应", examples),
        serverFrame("TaskDetailSnapshot", "04 TUI 快照响应", examples),
        serverFrame("ReplaySnapshot", "04 TUI 快照响应", examples),
        serverFrame("ReplayDetailSnapshot", "04 TUI 快照响应", examples),
        serverFrame("ThoughtSnapshot", "04 TUI 快照响应", examples),
        serverFrame("CrystalSnapshot", "04 TUI 快照响应", examples),
        ...
        clientFrame("EventUnsubscribe", "05 事件流", [], examples),
        serverFrame("EventPublish", "05 事件流", examples),
        serverFrame("ExecutiveLoopPausedEvent", "05 事件流", examples),
        serverFrame("ExecutiveLoopResumedEvent", "05 事件流", examples),
    ];
}
...
function buildApifoxProject(frames: FrameExample[], examples: Record<string, JsonRecord>): JsonRecord {
    const folders = Array.from(new Set(frames.map((frame) => frame.folder))).map((folder) => ({
        ...
        items: frames
            ...
            .map((frame) => ({
                name: `${frame.direction === "client->server" ? "WS 发送" : "WS 期望"} / ${frame.name}`,
                type: "api",
                api: {
                    protocol: "websocket",
                    method: "GET",
    ...
    return {
        ...
        info: {
            ...
            description:
                "从 canonical /ws OpenAPI examples 生成的 Apifox WebSocket 测试集合。真实 HTTP surface 仍只有 GET /health 和 WS /ws。",
