...
- 失败时返回上一次成功结果，并在 `mcp.tool.catalog.built` 事件里写入 `failedServers` / `staleServers`，避免单点 server 抖动阻塞整轮。
...
- 旧式 SSE 双端点已有客户端兼容，会话建立阶段现在有一次指数退避重试；`tools/call` 在传输/协议失败时也会重开一次 session 重试。`bun run smoke:recovery` 已覆盖本地 mock 的短暂断链与长结果回灌，并与 local working memory WAL/snapshot 恢复同属恢复门禁；但仍缺真实第三方 MCP server 的长期断链回归。
