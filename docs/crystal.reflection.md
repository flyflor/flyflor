...
- `src/fch/crystal/reflection/index.ts` — `CrystalReflectionComponent`，负责 candidate / atom / Gem 的确定性结晶规则；保留少量旧函数导出作为兼容薄壳
- `src/fch/crystal/gems/index.ts` — `CrystalGemComponent` / `InMemoryCrystalMemoryStore`，内部 Gem 模块边界
- `src/fch/crystal/memory/index.ts` — `CrystalMemoryComponent` 对外兼容门面 / `LocalCrystalMemoryStore`
