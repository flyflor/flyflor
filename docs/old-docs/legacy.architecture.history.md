...
| # | 维度 | 工程实现 |
| --- | --- | --- |
| 1 | 单文件大脑 | `~/.flyflor/brain.db`：event/state 分离、append-only、单库 + 时间字段 + 索引；月级冷归档自动运行 |
| 2 | 显式协作 | Ask 是一等公民：模型在不确定时**中断式**反问；reply / ask 在同轮二选一 |
| 3 | 思维边界 | Codename：用户显式 `@代号` 锚点；频次 + 衰减自然上浮为常驻；可升格为 Project |
| 4 | 不丢失的未完事项 | Continuation Context：Ask 中断时落 brain.db 的一种特殊 episode；用户可见、可主动 resume、自然衰减 |
...
| D6 | Continuation Context 作为未完事项快照 | 复用 episode + AtomScore 框架；用户可见、可 resume / drop / pin |
...
| D10 | 行为快照 | 每轮写 append-only behavior snapshot，保存输入、触发、输出的结构化预览；后续用户纠正写 correction 证据 |
| D11 | 提示词优先级冲突表 | system prompt 明确告诉模型：用户指令 > 追问状态 > 稳定身份偏好 > 当前工作区事实 > 未完成旧上下文 > 长期记忆/技能/摘要 > 情绪与休眠提示 |
...
```jsonc
{
  "memory": {
    "tuning": {
      ...
      "idle":        { "idleMinutes": 10, "_keepGatewayListening": true },
      "atomScore":      { "weights": { "recency": 0.35, "access": 0.15, "successPrior": 0.35, "fanout": 0.15 },
                          "visibilityThreshold": 0.65 },
      "continuation":          { "maxChainDepth": 5, "pinHalflifeMultiplier": 3.0 },
      "brainDb":        { "archiveAfterMonths": 3, "archiveIntervalHours": 24, "vacuumIntervalDays": 14 }
    }
  }
}
```
...
- AtomScore 权重 / brainDb / continuation 调参属内部参数，**不在 CLI / README 文档化**，避免误调。
...
~/.flyflor/
  ...
  identity/
    identity.md                         # 人格画像（人 + agent 共写，agent 仅 append）
    user.md                         # 用户偏好
...
CREATE TABLE memory_events (
  ...
  parent_id     TEXT,                 -- continuation 链 / ask-answer 关联
  embedding_id  TEXT,
...
CREATE TABLE memory_state (
  ...
  resumed_at      INTEGER,            -- continuation 专用
...
> 关键点：
> - **Continuation 不是新概念，是 `memory_events.type = 'continuation-context'`** 的一种。复用 AtomScore / decay / 召回 / gem 升格通路，零新机制。
> - **Ask-Answer pair 也是 events**，`type = 'ask-answer-pair'`，`parent_id` 指向触发它的 continuation。
...
- 默认权重见配置块；调参走内部脚本。
- inbox / continuation / pinned 的 recency / decay 走不同参数化：
    - inbox：`recencyDecay × inbox.decayMultiplier`（淡出快）
    - continuation.pinned：`recencyDecay × (1 / continuation.pinHalflifeMultiplier)`（淡出慢）
...
| `ScopeScaffolder` | 入口由"用户显式 + cluster 自动"改为"codename 升格 + 用户显式"，scaffolder 本身不动 |
...
- 链深度硬上限：`continuation.maxChainDepth`（默认 5）；超过 → runtime 强制 reply + episode 标 `excessive_clarification_loop`，作为"模型问太多了"的反馈信号。
...
## Continuation Context（D6）— 未完事项的可视副本

### 落地形态

Continuation = `memory_events.type = 'continuation-context'` 的一行，content 字段 JSON 携带：
...
- **可见**：TUI 侧栏按 codename 分组列出 continuation；CLI `flyflor continuation list / show / resume / drop / pin`；渠道 `/continuations` 命令。
- **可见阈值**：复用 `atomScore.visibilityThreshold`，默认 0.65；`--all` 显示全部。
- **Fork / Fresh 自决**：用户来新消息时，若当前 codename 有高分 continuation，prompt 注入 `[continuation-hint]`；模型同轮 `kind: 'fork' | 'fresh'` 决定是否恢复。零字符匹配。
- **显式 resume**：用户 `flyflor continuation resume <id>` 或 TUI 点 [resume] → 跳过模型自决，强制 fork。
- **Pin**：`continuation pin` 把 `memory_state.decay_score` 半衰期 × `continuation.pinHalflifeMultiplier`（默认 3.0），**不冻结**，仍参与 gem 升格证据计算。
- **Resume 成功后保留**：标 `resumed_at`，`importance` 拉回峰值；多次成功 resume 的 continuation 是 gem 升格的高价值证据。
- **Abandoned 不进晶体**：被用户新消息 cancel 的 ask，对应 continuation 不参与 gem 升格，但 `abandoned` 计数进 dream 反馈面板。
...
- 候选选择只用**已记录的 negative 信号**：用户显式纠正、连续工具失败、continuation abandoned 率、`contradicts` 链接计数。
...
| R14 brain.db 月度归档自动化 | ✅ done — `src/fch/hippocampus/memory/brain/index.ts` 统一 admin 脚本与 runtime；`MemoryModule.runBrainArchiveOnce` 只搬 `state=archived` 且早于 cutoff 的事件和同月 summary，发布 `MemoryBrainArchiveCompleted/Failed`；`BackgroundScheduler` 场景避开 summary / dream busy，降级场景由根 timer 继续维护 |
