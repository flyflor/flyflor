...
- owner: `main-codex`
- scope: `PLAN.md`, Executive tool runtime ASK promotion, runtime subagent needs-user propagation, ASK protocol shape, Executive pause presentation, external tools app-relative command resolution, xtools installer defaults
- `PLAN.md` now exists as the append-only execution ledger for the full multi-phase plan; future work may only change task statuses or append amendments
- `ExecutiveToolRuntime` exposes `onExecutionAskRequired`, allowing runtime-owned structured tool results to pause the parent loop with ASK without hard-coding subagent behavior into Executive
- `subagent.batch` preserves child `askRequired` in `SubagentChildResult` / `SubagentBatchResult`; child `needs_user` now returns a parent ASK instead of an ordinary reply with failed tool metadata
- ASK contracts now include authority/source/resumePolicy, per-question recommended choices, fixed `other`, execution patches, and candidate policy fields while keeping old ASK payloads compatible
- Executive pause ASK renders three structured questions: execution strategy, budget policy, and subagent policy; old root choices remain for existing clients
- `FlyflorPaths` has optional `appRoot`; external tool manifests support schema v2 and `cwd:"app"`; repository xtools manifests and installers now persist app-relative `./tools/packages/...` commands
- absolute sidecar commands in external tool manifests are marked unavailable, keeping registry files portable

Remaining phases:

- `AskComponent` split into parser/normalizer/policy/presentation/ledger
- Durable Job component, `brain.db` execution-job events, and `/ws` job read model
- external tool stability state machine, upgrade staging/rollback, ASK/tool-stability integration, and Crystal evidence closure

Validation:

- `bun test tests/ask.parse.test.ts tests/ask.wire.test.ts tests/executive.tool.runtime.test.ts tests/skill.mcp.test.ts --timeout 30000`
- `bun test tests/external.tools.test.ts tests/install.script.test.ts --timeout 30000`
- `bun run check`

## 2026-05-25 Executive ASK Job Ledger V1

Accepted continuation slice:

- owner: `main-codex`
- scope: ASK owner split, Execution Job v1, subagent job metadata, brain.db execution-job ledger
- ASK parsing/normalization/presentation now belongs to `src/cognitive/hippocampus/ask` with `AskComponent`; runtime turn helpers remain compatibility adapters
- `subagent.batch` now creates a parent Execution Job and child jobs, exposes jobId/childJobId/progress in reply metadata, and preserves needs-user ASK linkage
- Execution Job lifecycle writes append-only `memory_events.type='execution-job'` rows with bounded structured summaries only
- brain.db remains ledger/query/replay/audit/detail; job ledger events do not become prompt atoms and do not store complete prompts or large tool outputs

Validation:

- `bun test tests/ask.parse.test.ts tests/ask.reply.test.ts tests/ask.wire.test.ts tests/ask.normalizer.test.ts tests/ask.presentation.test.ts tests/executive.tool.runtime.test.ts tests/skill.mcp.test.ts tests/external.tools.test.ts tests/install.script.test.ts --timeout 30000`
- `bun run check`
- `git diff --check`

## 2026-05-26 Executive ASK Tools Complete

Accepted final implementation closure:

- owner: `main-codex`
- scope: `PLAN.md` Phase 0-15, socket execution job query, external tool stability, external package upgrade transaction, ASK/tool-stability handoff, Crystal candidate evidence, docs/OpenAPI/Apifox
- `execution.job.list` and `execution.job.detail.get` now return `execution.job.snapshot` from `brain.db` execution-job ledger only; the query path does not call Runtime, model, tools, or prompt/context assembly
- external tool descriptors carry stability snapshots with discovery, manifest, path, version, probe, runtime, sandbox, upgrade, and effective states
- Tool Plan keeps unavailable descriptors registered for diagnostics while hiding unavailable/disabled/upgrading tools from the model
- package manager staging writes package metadata under `.staging` and a relative-path `external.tools.jsonc.next`; apply preserves the previous package under `.previous`
- unavailable external tool stability can pause the Executive loop with a `tool-stability` ASK
- ASK/job/tool-stability structured evidence enters Crystal candidate input, but Gem promotion still requires `CrystalReflectionComponent` quality gate
- docs, canonical OpenAPI, and Apifox generated artifacts now include multi-question ASK, execution job read models, relative path/stability/upgrade contracts, and Crystal candidate boundaries
