...
迁移期说明：当前源码物理路径已迁移到 `src/executive`；旧执行层物理路径已移除。文档中的 Executive 指当前层名，新代码只能从 `src/executive` 进入能力层。
...
| Trust | 根据 channel、sender、group、turn/request context、workspace、sandbox、approval、secrets 和 permission cap 决定本次是否可执行 | 不做自然语言语义判断 |
...
`scope` 是轻量集合，初始约定为 `core`、`chat`、`subagent`、`memory`、`background`、`channel`、`local`、`workspace`、`debug`。scope 只表达使用场景，不表达危险等级。
...
| 来源 | 示例 | 入口规则 |
| --- | --- | --- |
| core | local files, git, todo, cron, delegate, screenshot, lsp | Explicitly registered by the composition root; not discovered by prompt text |
...
| user | `~/.flyflor/.config/tools/*.jsonc` 或 workspace-local manifest | 必须声明 schema、permission、cwd、env、输出限制 |
...
1. 收集已注册 descriptors。
2. 按 config、auth、secrets、channel、sender/group、workspace、sandbox、platform availability 过滤。
3. 对可见工具排序并注入模型上下文。
...
Trust Policy 把结构化运行面转换成 trust context。调用点只声明当前 surface、是否本地、是否 workspace-scoped、是否 debug，Executive 决定默认 scope 和 permission cap：

| 场景 | 默认 scopes | 默认 permission cap |
| --- | --- | --- |
| 远程 channel / webhook | `core`、`chat`、`channel`、可选 `workspace` | `message` |
| 本地 CLI / TUI | `core`、`chat`、`local`、可选 `workspace` | `write` |
| 本地 debug | `core`、`chat`、`local`、`debug`、可选 `workspace` | `dangerous` |
| background / cron | `core`、`background`、可选 `workspace` | `network` |
...
| filesystem read/search | `read` | 可并发，跨 workspace/root 读访问需 Trust 决策 |
