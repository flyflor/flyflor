...
- Gateway 已有单进程 `InMemoryDedupStore` 与 `buildDedupKey(channel,messageId)`；多副本部署时需要新增独立共享 dedup Component 才能跨节点幂等。若未来恢复 Redis 后端，应继承 `RedisComponent` 表达原型边界，而不是把 gateway 专用 Redis 适配器塞回默认链路。
