...
- Chat does not attach custom `onMouseScroll` handlers to ScrollBox regions. OpenTUI owns wheel events, scroll acceleration, sticky-bottom state, and content translation; `src/command/tui/scrollbar.composition.ts` removes the built-in visual scrollbar renderables so Flyflor chrome does not paint a second scrollbar, and `src/command/tui/screen.composition.ts` pins all command TUI renderers to alternate screen so terminal scrollback/native scrollbars never become the viewport
