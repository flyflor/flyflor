...
- Chat does not attach custom `onMouseScroll` handlers to ScrollBox regions. Keyboard PageUp/PageDown/Home/End owns portable scrolling, while terminal-native selection stays available because Chat does not enable OpenTUI mouse mode. `src/command/tui/scrollbar.composition.ts` removes the built-in visual scrollbar renderables; the chat pane draws the target screenshot virtual rail (`▲`, dotted track, `██` thumb, `▼`).
- `src/command/tui/screen.composition.ts` pins command TUI renderers to alternate screen and clears terminal scrollback (`CSI 3 J`) before runtime warmup so Docker/provider startup output cannot seed the main-screen scrollback. Mouse tracking is owned only by OpenTUI renderer config; Flyflor must not emit duplicate all-motion tracking (`1003`) because iTerm2/Bash can flood stdin and trip the parser.
- `src/command/tui/renderer.composition.ts` is the only place that normalizes OpenTUI renderer defaults. Chat keeps `useMouse: false` so terminal selection and macOS scrollback behavior remain native; command navigators that enable mouse must still set `enableMouseMovement: false` because OpenTUI defaults this field to true.
...
- Chat message history and both right-rail stream panels use OpenTUI `stickyScroll` with `stickyStart: "bottom"`; streamed thinking / blackboard detail follows the latest output until the user uses keyboard scrolling
...
- Chat scroll behavior is a fixed terminal-safe contract: chat and side-panel scrollboxes use sticky bottom mode, PageUp/PageDown/Home/End are the portable scroll controls, and Chat starts in alternate-screen mode without OpenTUI mouse selection.
