...
- `src/command/tui/renderer.composition.ts` is the only place that normalizes OpenTUI renderer mouse defaults. Flyflor may enable `useMouse` for click, wheel, and selection, but it must set `enableMouseMovement: false`; OpenTUI defaults this field to true, which re-enables all-motion tracking even when Flyflor does not write terminal mouse escape sequences itself.
- iTerm2 shells that omit `COLORTERM` are normalized to `truecolor` only for renderer creation, then restored. This keeps macOS Bash color output on the truecolor path without mutating user config.
