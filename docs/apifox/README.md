...
这些 `/__apifox/ws/...` 路径不是 Flyflor 服务端接口，不允许在实现中新增对应 HTTP 路由。Postman Collection 里的 HTTP-looking request 只是导入承载体；不要直接点击这些条目的“发送”。真实测试方式始终是新建 Apifox WebSocket 接口，连接 `ws://127.0.0.1:8788/ws`，然后把 `Frame 示例-发送 / ...` 条目里的 raw JSON body 复制到 WebSocket message editor 发送。
