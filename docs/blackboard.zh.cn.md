...
- `src/agent/runtime/blackboard/index.ts` — `blackboard.route.md` 调用与 JSON 校验
...
sequenceDiagram
    ...
    RT->>BB: startTurn({scopeConstraintId, goal, workers, budget})
    BB->>Store: acquireLease(scopeConstraintId, ttl)
    alt 已有 lease
...
```json
// stdin：单行 JSON
{ "context": { "taskId", "workerName", "runtime", "requestId", "scopeConstraintId", "turnId" }, "input": {} }
...
- 同 scope constraint 同时只允许一个 blackboard turn；冲突时降级 direct 或返回「已有任务运行中」。
