...
| planning / memory write | `memory.task_plan.written` `memory.context_fork.written` `memory.replay_record.written` | 审计、历史回放提示、增量刷新提示 | 否，结构化快照仍读 `turn.final.reply.metadata.planning` |
