...
import type { InferenceEvent, ProtocolAdapter, ProtocolBuildContext, ProtocolStreamState, ProviderErrorShape, ProviderMessage, StreamingActionRequest } from '../types';
 ...
 */
function accumulateActionRequest(controller: ReadableStreamDefaultController<InferenceEvent>, state: ProtocolStreamState, delta: WireActionDelta): void {
 ...
 */
function finalizeActionRequests(controller: ReadableStreamDefaultController<InferenceEvent>, state: ProtocolStreamState): void {
...
/**
 * EN: OpenAI Responses wire protocol adapter (`openaiResponses`).
 * ZH: OpenAI Responses 线协议适配器（`openaiResponses`）。
 */
...
function providerErrorMessage(error: ProviderErrorShape | undefined, fallback: string, message?: string, code?: string): string {
    ...
    return resolvedMessage ? (resolvedCode ? `${resolvedCode}: ${resolvedMessage}` : resolvedMessage) : fallback;
}
