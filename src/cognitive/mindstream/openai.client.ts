...
import { ModelApiMode, ModelProviderId } from "../../protocol/index.ts";
...
export class OpenAICompatibleClient implements ModelClient {
    ...
    public async generate(messages: ModelMessage[], options: { signal?: AbortSignal } = {}): Promise<string> {
        ...
        const response = await fetchModelEndpoint(this.config, this.chatCompletionsPath(), {
            method: "POST",
    ...
    private chatCompletionsPath(): string {
        // DeepSeek's OpenAI-compatible endpoint is documented at the root
        // `/chat/completions` path, while generic relays keep the `/v1` prefix.
        return this.config.providerId === ModelProviderId.DeepSeek ? "/chat/completions" : "/v1/chat/completions";
    }
