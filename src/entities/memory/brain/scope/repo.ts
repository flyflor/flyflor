 ...
 * and memory components so this table cannot become an implicit continuity layer.
 */
export class BrainScopeRepo {
    ...
    public upsert(record: ScopeRecord): ScopeRecord {
        runQuery(
            ...
            query`INSERT INTO scopes (
                id, title, goal, project_dir, project_memory_dir,
                created_at, updated_at, last_used_at, use_count
            ) VALUES (
                ${record.id}, ${record.title}, ${record.goal ?? null},
    ...
    public list(input: { limit?: number } = {}): ScopeRecord[] {
