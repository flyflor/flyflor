...
export class BrainTaskPlanModel {
    ...
    private parseJsonArray(value: string, field: string): TaskPlanStepRecord[] {
        const parsed = JSON.parse(value) as unknown;
        if (!Array.isArray(parsed)) {
            throw new Error(`${field} must be a JSON array.`);
        }
        if (parsed.some((item) => !this.isTaskPlanStepRecord(item))) {
            throw new Error(`${field} must contain only valid task plan step records.`);
        }
        return parsed;
    }
