...
} from "../../../../cognitive/crystal/memory/vector.index.ts";
import type {
    GemRecord,
    GraphRecallInput,
    MemoryNodeRecord,
    SummaryEmbeddingInput,
} from "../../../../cognitive/hippocampus/memory/graph/types.ts";
...
export class SQLiteGraphModel {
    ...
    public resolveEmbedding(input: number[], fallbackText: string, dimensions: number): number[] {
        if (Array.isArray(input) && input.length > 0) {
            if (input.some((value) => typeof value !== "number" || !Number.isFinite(value))) {
                throw new Error("Memory graph embedding must be a finite number array.");
            }
            return input;
        }
    ...
    private parseJsonArray(value: string): string[] {
        ...
        if (!Array.isArray(parsed)) {
            throw new Error("Memory graph row expected a JSON array.");
        }
        if (parsed.some((item) => typeof item !== "string")) {
            throw new Error("Memory graph row expected a string array.");
        }
        return parsed;
    }
    ...
    private parseJsonNumberArray(value: string): number[] {
        ...
        if (parsed.some((item) => typeof item !== "number" || !Number.isFinite(item))) {
            throw new Error("Memory graph row expected a finite number array.");
        }
        return parsed;
    }
    ...
    private parseJsonRecord(value?: string | null): Record<string, unknown> | undefined {
        ...
        if (!this.isRecord(parsed)) {
            throw new Error("Memory graph row expected a JSON object.");
        }
        return parsed;
    }
