...
import type { ChannelName, GatewayMessage, RuntimeContext } from "../shared/core/types.ts";
import { event, type EventSink } from "../shared/events/index.ts";
import { Gateway } from "../shared/fcp/decorators.ts";
...
export class GatewayServer {
    ...
    start(): void {
        ...
        this.events.publish(event("gateway.start", { url: server.url.toString() }));
    ...
    private async dispatch(message: GatewayMessage) {
        ...
        this.events.publish(event("gateway.message.received", { channel: message.route.channel }, context.requestId));
