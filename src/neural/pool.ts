...
export class AgentPool extends FComponent {
    ...
    /** EN: Creates an unbound persistent-person pool. ZH: 创建一个尚未绑定皮层总线的持久人物池。 */
    public constructor() {
        super();
        this.agents = new Map();
        this.active = '';
    ...
    /** EN: Validates configured identities before the pool is exposed to Synapse. ZH: 在人物池暴露给 Synapse 前验证已配置身份。 */
    @Init()
    public init(): void {
    ...
    /** EN: Binds the sole cortical bus and creates every configured person once. ZH: 绑定唯一皮层总线，并一次性创建全部已配置人物。 */
    ...
    /** EN: Validates and copies one configured profile without mutating shared configuration. ZH: 验证并复制一个已配置 profile，且不修改共享配置。 */
