 ...
 * `user`/`answer` packets are handled by FSocket itself; any other action is
 * reflected onto a same-named Controller method.
 * ZH: Unix socket 协议上约定的 IPC action 名。入站 `user`/`answer` 包由 FSocket
 * 自身处理；其余 action 会反射到 Controller 上的同名方法。
 */
export enum SocketEvent {
    ...
    Open = 'open',
    /** EN: Inbound user message turned into an Awareness stimulus. ZH: 入站用户消息，会转化为 Awareness 刺激。 */
    User = 'user',
...
export class FSocket extends FlyFlor {
    ...
    /** EN: Attention gate that receives every inbound stimulus. ZH: 接收所有入站刺激的注意门。 */
    @Inject()
    ...
    /** EN: Neural cortex, attached after construction to break the init cycle. ZH: 神经皮层，构造后挂载以打破初始化环。 */
     ...
     * Awareness to forget it, and removes both indexes.
     * ZH: 拆除离开的说话人：丢弃其 Connection 状态，通知 Awareness 遗忘它，并移除
     * 两个索引。
     */
     ...
     * each is decoded, and `user`/`answer` packets go to Awareness while other
     * actions dispatch to a same-named Controller method. One malformed packet
     * is logged and skipped without aborting the frames that follow it.
     * ZH: 入站字节处理器。字节可能分包到达或多帧粘连；Connection 负责重组出完整
     * 帧，逐帧解码后，`user`/`answer` 包交给 Awareness，其余 action 反射派发到
     * Controller 同名方法。单个畸形包只记录并跳过，不会中断其后的帧。
     */
