...
export class BrainEventRepo {
    ...
    public list(input: BrainEventListInput = {}): MemoryEventRecord[] {
        ...
        const statusFilter = statusFilterParams(input.statusIn);
    ...
    public listActiveGhosts(
        ...
        const codenameFilter = codenameFilterParams(options.codenameId);
        const rows = allQuery<BrainEventRow>(
            this.db,
            query`SELECT e.* FROM memory_events e
                LEFT JOIN memory_state s ON s.event_id = e.id
    ...
    public patchGhostContent(eventId: string, patch: Record<string, unknown>): MemoryEventRecord | null {
        ...
        const current = parseObjectContent(row.content, `Invalid ghost-context content JSON for event ${eventId}`);
...
function statusFilterParams(statuses: MemoryEventStatus[] | undefined): StatusFilterParams {
...
function codenameFilterParams(codenameId: string | null | undefined): CodenameFilterParams {
...
function rowToEvent(row: BrainEventRow): MemoryEventRecord {
    return {
        id: row.id,
        ts: row.ts,
...
function parseObjectContent(value: string, errorPrefix: string): Record<string, unknown> {
    let parsed: unknown;
    try {
