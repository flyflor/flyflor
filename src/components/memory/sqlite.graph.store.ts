...
export class SQLiteGraphStore extends GraphComponent implements MemoryGraphStore {
    ...
    public async initialize(): Promise<void> {
        ...
        for (const row of database.query("SELECT * FROM graph_summary_embeddings").all() as SummaryEmbeddingRow[]) {
            const record = rowToSummaryEmbedding(row);
    ...
    public async upsertEpisode(input: EpisodeNodeInput): Promise<void> {
        ...
        const record: MemoryEpisodeRecord = {
            ...
            embedding: resolveEmbedding(input.embedding, `${input.text} ${input.concepts.join(" ")}`, this.vectorDimensions),
    ...
    public async upsertMemoryNode(input: MemoryNodeInput): Promise<void> {
        ...
        const record: MemoryNodeRecord = {
            ...
            embedding: resolveEmbedding(input.embedding, `${input.summary} ${input.symbols.join(" ")}`, this.vectorDimensions),
    ...
    public async listContradictionPairs(input: {
        ...
        for (const seed of seeds) {
            ...
            const neighbors = [...this.memoryNodes.values()]
                ...
                .map((row) => ({
                    row,
                    cosine: cosine(seed.embedding, row.embedding),
                }))
    ...
    public async applyGemDriftRepair(input: {
        ...
        if (Array.isArray(input.newSymbols)) gem.symbols = sanitizeSymbols(input.newSymbols);
        ...
        if (typeof input.confidenceMultiplier === "number") {
            gem.confidence = clamp01(gem.confidence * input.confidenceMultiplier);
        }
    ...
    public async applyMemoryReinforce(input: {
        ...
        if (!this.config.dbFile) return false;
        await this.initialize();
        const multiplier = clamp(input.importanceMultiplier, 0.5, 1.5);
        if (input.table === "memory_node") {
            const row = this.memoryNodes.get(input.id);
            if (!row) return false;
            row.importance = clamp01((row.importance ?? 0) * multiplier);
            ...
            if (!row) return false;
            row.importance = clamp01((row.importance ?? row.confidence) * multiplier);
            row.recallCount = (row.recallCount ?? 0) + 1;
            row.updatedAt = input.nowMs;
    ...
    public async applyContradictionAudit(input: {
        ...
        if (!this.config.dbFile) return false;
        await this.initialize();
        const multiplier = clamp(input.confidenceMultiplier, 0.3, 1.0);
        const delta = Math.max(0, Math.min(5, Math.floor(input.contradictionDelta)));
        let ok = false;
        if (input.table === "memory_node") {
            const row = this.memoryNodes.get(input.id);
            if (!row) return false;
            row.confidence = clamp01(row.confidence * multiplier);
    ...
    public async applyReconsolidation(input: {
        ...
        if (Array.isArray(input.mergedSymbols)) {
            const symbols = sanitizeSymbols(input.mergedSymbols);
    ...
    private searchMemoryNodes(input: GraphRecallInput, limit: number): MemoryNodeRecord[] {
        ...
        const queryEmbedding = buildQueryEmbedding(input, this.vectorDimensions);
        const scored = rows
            ...
            .map((row) => ({
                row,
                score: scoreMemoryNode(row, queryEmbedding, symbols),
            }))
    ...
    private searchGems(input: GraphRecallInput, limit: number): GemRecord[] {
        ...
        const scored = rows
            ...
            .map((row) => ({
                row,
                score: scoreGem(row, queryEmbedding, symbols),
            }))
            .filter((entry) => entry.score > 0)
    ...
    private async relate(
        ...
        const record: GraphEdgeRecord = {
            ...
            metadata: normalizeRecord(content),
            createdAt: Date.now(),
        };
        this.edges.set(record.id, record);
        this.requiredDatabase()
...
function rowToEpisode(row: EpisodeRow): MemoryEpisodeRecord {
    return {
        id: row.id,
        userId: row.user_id,
        text: row.text,
...
function rowToMemoryNode(row: MemoryNodeRow): MemoryNodeRecord {
    return {
        id: row.id,
        userId: row.user_id,
...
function rowToGem(row: GemRow): GemRecord {
    return {
        id: row.id,
        userId: row.user_id,
        symbols: parseJsonArray(row.symbols_json),
...
function rowToSummaryEmbedding(row: SummaryEmbeddingRow): SummaryEmbeddingInput {
    return {
        id: row.id,
        userId: row.user_id,
...
function rowToSnapshot(row: GemSnapshotRow): GemSnapshotRecord {
    return {
        id: row.id,
        gemId: row.gem_id,
        userId: row.user_id,
...
function rowToEdge(row: GraphEdgeRow): GraphEdgeRecord {
    return {
        id: row.id,
...
function buildQueryEmbedding(input: GraphRecallInput, dimensions = DEFAULT_CRYSTAL_VECTOR_DIMENSIONS): number[] {
    if (input.embedding && input.embedding.length > 0) {
        return input.embedding;
    }
    return embedCrystalText(buildQueryText(input), dimensions);
}

function buildQueryText(input: GraphRecallInput): string {
    return [...(input.symbols ?? [])].filter(Boolean).join(" ");
}

function scoreMemoryNode(row: MemoryNodeRecord, queryEmbedding: number[], querySymbols: string[]): number {
    const textSimilarity = cosine(queryEmbedding, row.embedding);
    const symbolOverlap = overlapRatio(querySymbols, row.symbols);
    const confidence = clamp01(row.confidence);
    const freshness = freshnessScore(row.lastAccessedAt ?? row.updatedAt);
    return textSimilarity * 0.72 + symbolOverlap * 0.18 + confidence * 0.08 + freshness * 0.02;
}

function scoreGem(row: GemRecord, queryEmbedding: number[], querySymbols: string[]): number {
    const textSimilarity = cosine(queryEmbedding, row.embedding);
    const symbolOverlap = overlapRatio(querySymbols, row.symbols);
    const confidence = clamp01(row.confidence);
    const freshness = freshnessScore(row.lastVerifiedAt ?? row.updatedAt);
    return textSimilarity * 0.72 + symbolOverlap * 0.18 + confidence * 0.08 + freshness * 0.02;
}

function resolveEmbedding(input: number[], fallbackText: string, dimensions: number): number[] {
...
function normalizeRecord(value?: Record<string, unknown>): Record<string, unknown> | undefined {
    if (!value) return undefined;
    return Object.fromEntries(Object.entries(value).filter(([, item]) => item !== undefined)) as Record<string, unknown>;
}
...
function freshnessScore(updatedAt: number): number {
