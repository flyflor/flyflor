...
export class Brain extends FAgentAtom<CallosumSignal> {
    ...
    constructor(
        public override agentConfig: FAgentProfileConfiguration,
        public override synapse: Synapse,
    ...
    public async run(input: AgentTurnInput): Promise<void> {
        ...
        //     });
        //     void this.callosum
        //         .run(this.memory.buildMessage(), (messages) => this.intelligence.completeText(messages))
        //         .catch((error) => {
        //             subscription.unsubscribe();
        //             reject(error);
        //         });
    ...
    public async research(data: CallosumSignal, input: AgentTurnInput) {
        ...
        // const outcome = await this.researcher.run(
        //     this.researchMessages(data.chunk, input),
        //     (signal) => {
    ...
    private researchMessages(content: string, input: AgentTurnInput) {
    ...
    private researchRuntimeContext(input: AgentTurnInput) {
        // const lines: string[] = [];
        ...
        //     lines.push(this.runtimeText.text(RESEARCH_WORKING_DIRECTORY_TEXT_KEY, { workingDirectory: input.workingDirectory }));
        // }
