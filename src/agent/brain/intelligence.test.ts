...
import { AgentChatRole, Intelligence, type AgentChatMessage, type IntelligenceRequest } from './intelligence';
...
describe('Intelligence', () => {
    test('streams JSON deltas until a structured finish reason', async () => {
        ...
        globalThis.fetch = (async (input: FetchInput, init?: FetchInit) => {
            ...
            return new Response(bodyFromText(
        ...
        const intelligence = await useContainer().getAsync(Intelligence, configuration());

        await expect(intelligence.complete(messages())).resolves.toBe('hello');
        expect(url).toBe('https://example.test/chat/completions');
        expect(requestBody).toMatchObject({ stream: true, max_tokens: 32 });
    });
    ...
    test('tries a v1 endpoint when the configured base URL omitted it', async () => {
        ...
        await expect(intelligence.complete(messages())).resolves.toBe('ok');
        expect(urls).toEqual([
    ...
    test('reuses one Intelligence instance across turns with explicit turn messages', async () => {
        ...
        globalThis.fetch = (async (_input: FetchInput, init?: FetchInit) => {
            ...
            return new Response(bodyFromText(
                `data: {"choices":[{"delta":{"content":"${last}"},"finish_reason":null}]}\n\n`,
                'data: {"choices":[{"delta":{},"finish_reason":"stop"}]}\n\n',
            ));
        }) as unknown as typeof fetch;
        ...
        await expect(intelligence.complete(messages([{ role: AgentChatRole.User, content: 'first' }]))).resolves.toBe('first');
        await expect(intelligence.complete(messages([{ role: AgentChatRole.User, content: 'second' }]))).resolves.toBe('second');
        expect(seenMessages).toEqual([
            [{ role: AgentChatRole.User, content: 'first' }],
            [{ role: AgentChatRole.User, content: 'second' }],
        ]);
    });
    ...
    test('uses Responses streaming when chat-completions protocol is unavailable', async () => {
        ...
        await expect(intelligence.complete(messages())).resolves.toBe('response');
        expect(urls).toEqual([
            'https://example.test/chat/completions',
            'https://example.test/v1/chat/completions',
    ...
    test('uses Responses streaming when chat-completions returns non-streaming JSON', async () => {
        ...
        await expect(intelligence.complete(messages())).resolves.toBe('stream');
        expect(urls).toEqual([
            'https://example.test/chat/completions',
            'https://example.test/v1/chat/completions',
            'https://example.test/responses',
    ...
    test('does not duplicate v1 when the configured base URL already includes it', async () => {
        ...
        const intelligence = await useContainer().getAsync(Intelligence, configuration(modelConfiguration('https://example.test/v1')));
    ...
    test('respects configured protocol order', async () => {
        ...
        const intelligence = await useContainer().getAsync(Intelligence, configuration(modelConfiguration('https://example.test', [
    ...
    test('supports ollama structured done events', async () => {
        ...
        const intelligence = await useContainer().getAsync(Intelligence, configuration(modelConfiguration('http://127.0.0.1:11434', [
            { name: FModelProtocolName.Ollama },
        ])));

        await expect(intelligence.complete(messages())).resolves.toBe('hello world');
    });
    ...
    test('supports anthropic message stop events', async () => {
        ...
        const intelligence = await useContainer().getAsync(Intelligence, configuration(modelConfiguration('https://api.anthropic.com', [
            { name: FModelProtocolName.AnthropicMessages },
        ])));

        await expect(intelligence.complete(messages())).resolves.toBe('hi');
    });
    ...
    test('includes the system prompt in Gemini system_instruction', async () => {
        ...
        const intelligence = await useContainer().getAsync(Intelligence, configuration(modelConfiguration(
            'https://generativelanguage.googleapis.com',
            [{ name: FModelProtocolName.GoogleGeminiGenerateContent }],
        )));

        await expect(intelligence.complete(input)).resolves.toBe('ok');
    ...
    test('accepts Bedrock JSON streaming responses and sends bearer auth', async () => {
        ...
        await expect(intelligence.complete(messages())).resolves.toBe('bedrock');
    ...
    test('errors when the stream ends without a structured finish reason', async () => {
        ...
        await expect(intelligence.complete(messages())).rejects.toThrow('structured finish_reason');
    });
    ...
    test('errors on non-JSON stream data', async () => {
        ...
        await expect(intelligence.complete(messages())).rejects.toThrow('non-JSON stream data');
    });
    ...
    test('errors when no turn messages were published', async () => {
        ...
        await expect(intelligence.complete([])).rejects.toThrow('messages are missing');
    });
});
...
function messages(
    input: AgentChatMessage[] = [{ role: AgentChatRole.User, content: 'hello' }],
): AgentChatMessage[] {
    return input;
}
