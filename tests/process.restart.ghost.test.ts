...
import { GhostContextReason, type ModelClient } from "../src/protocol/contracts/index.ts";
...
describe("LF-R4 process-restart ghost recovery", () => {
    test("warmup picks up leftover inflight sentinel and writes a process-restart ghost", async () => {
        try {
            ...
            try {
                ...
                expect(c.reason).toBe(GhostContextReason.ProcessRestart);
                ...
                const recorded = events.events.find(
                    (e) =>
                        e.type === RuntimeEventType.MemoryGhostRecorded &&
                        e.payload?.reason === GhostContextReason.ProcessRestart,
                );
