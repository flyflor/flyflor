...
describe("pending scope offer DAO + nudge lifecycle", () => {
    test("upsert / get / ttl decrement / delete", async () => {
        ...
        const got = await store.getScopeOffer("scope:u1");
        expect(got).toBeDefined();
        expect(got?.title).toBe("Refactor router");
        expect(got?.relatedIds).toEqual(["e1", "e2", "e3"]);

        const r1 = await store.decrementScopeOfferTtl("scope:u1");
        expect(r1).toBe(1);
        const r2 = await store.decrementScopeOfferTtl("scope:u1");
        expect(r2).toBe(0);
        const after = await store.getScopeOffer("scope:u1");
        expect(after).toBeUndefined();
    });

    test("noteScopeOfferTurn: explicit trigger consumes; non-trigger decrements then expires", async () => {
        const config = await testConfig();
        ...
        await memory.noteScopeOfferTurn("scope:u2", false);
        expect(sink.events.find((e) => e.type === RuntimeEventType.MemoryScopeOfferExpired)).toBeDefined();
        expect(await store.getScopeOffer("scope:u2")).toBeUndefined();
        ...
        await memory.noteScopeOfferTurn("scope:u3", true);
        expect(sink.events.find((e) => e.type === RuntimeEventType.MemoryScopeOfferConsumed)).toBeDefined();
        expect(await store.getScopeOffer("scope:u3")).toBeUndefined();
    });
});

async function testConfig() {
    const root = await mkdtemp(join(tmpdir(), "flyflor-scope-offer-"));
    tempRoots.push(root);
