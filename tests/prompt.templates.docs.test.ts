...
describe("prompt template docs generator", () => {
    test("matches the checked-in README section", async () => {
        ...
        const readmeZhCn = await readFile(join(import.meta.dir, "..", "README.zh.cn.md"), "utf8");
        const checkedInZhCn = extractPromptTemplateSection(readmeZhCn);
        expect(generatedZhCn).toBe(checkedInZhCn);
    });
