...
- 当前分支只拥有 runtime-executive-ws 切片：`src/agent/runtime/**`、`src/agent/gateway/**`、`src/executive/**` 与其直接相关的测试/文档/smoke。
- 未经主线协调者批准，不修改 `src/cognitive/hippocampus/memory/**`、`src/cognitive/hippocampus/scope/**`、`src/cognitive/crystal/**`。
- 交还前只更新本 worktree 的 `TODO.md` / `LOGS.md`；主线 handoff 文档由协调者分支维护。
