...
Delegate command lookup 支持：

- 用户显式配置的绝对 command；
- `./tools/packages/...` 这类 app/project 相对 command；
- PATH command；
- `.cmd`、`.exe`、`.bat`、`.com` 这类 PATHEXT 风格可执行后缀。

这样 Windows 风格 delegate 不需要在 manifest 中硬编码平台后缀也能工作。默认真实 manifest 仍保持可移植：安装包 command 继续是 app-relative，且 `tools: []`，只有显式 opt-in 后才进入模型工具面。
