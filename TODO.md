...
| M-08 | project cluster 路径闭环 Phase A 已落地：SQLite `pending_project_offer` 表 + DAO（upsert/get/decrementTtl/delete）；`MemoryModule.sweepProjectClusters(userId)` 手动触发（从 Redis context ring 聚合 concept → detectClusterCandidate → 写 offer 发 `MemoryProjectOfferProposed`）；`buildPrompt` 注入 Hermes-style nudge；commitTurn 末端 `noteProjectOfferTurn`：显式 projectIntent 触发即 consume 发 `MemoryProjectOfferConsumed`，否则 ttl-1，0 时发 `MemoryProjectOfferExpired`。**Phase B 待补**：BackgroundScheduler 周期 sweep（setInterval）+ 事件接入 doctor 诊断 | P1 |
