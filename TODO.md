...
| ID       | 描述                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 优先级 |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| G-01     | BlueBubbles / iMessage / DingTalk / Email / HomeAssistant / Line / Mattermost / Matrix / QQ / Signal / Slack / SMS / WeCom / WhatsApp / Zalo 仅是 `HttpPlatformAdapter` 占位 ✅ partial — Slack（HMAC-SHA256 `x-slack-signature` + 5min 时间窗 + url_verification + `channel_type==="im"`→direct / `mpim/channel/group`→group + thread_ts + `files[]` → `GatewayAttachment`，新 `src/agent/gateway/channels/slack.ts`），BlueBubbles/iMessage（共享密码 query/header + chats[].style=group + attachments[] mimeType + 群组识别，新 `bluebubbles.ts`），Telegram（增加 photo / document / voice → `GatewayAttachment` + `message_thread_id`），三套适配器在 `channels/index.ts` 优先选用；DingTalk / Email / HomeAssistant / Line / Mattermost / Matrix / QQ / Signal / SMS / WeCom / WhatsApp / Zalo 仍走 `HttpPlatformAdapter` 占位，留待按需逐个补 | P2     |
