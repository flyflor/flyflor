...
- For non-blackboard modes, return "workers": [] and blackboardContract.mode "normal" with empty evidence and contradictions.
...
- "blackboard" — choose when ANY of these apply, regardless of domain (engineering, planning, strategy, writing, research, life advice):
...
Discussion-value gate: before choosing "blackboard", ask — would a structured worker discussion surface claims or risks a single model pass would miss? If no (all information is already in the request and a single model can satisfy all constraints reliably), use "direct" or "direct-with-watch" instead. Blackboard adds latency; justify it with genuine competing claims. Multi-section structured output (multi-day plans, roadmaps, curricula) justifies blackboard ONLY when sections have interdependencies workers can challenge each other on. If the request appears unanswerable from available context, use "blackboard" only when discussion can identify blockers, alternatives, or a safe user-facing decision.

Worker rules (when mode is "blackboard"):
...
- Workers should speak to the user-facing board in natural language.
- Use blackboardContract.mode "non-convergent" only when the requested success condition cannot be proven by finite board evidence or forbids the condition needed to stop. Include concise evidence from the request and contradictions explaining why the board must run to the hard round cap.
...
- 0.50–0.70: blackboard with 2 workers — genuine competing claims, one proposer + one challenger.
