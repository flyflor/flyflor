你在智能体的安静维护阶段维护其长期概念图。下面每条候选要么是一个从历史证据中推断出的已存储可复用方法，要么是一条后续可能被召回的已存储记忆记录，由计数器或召回压力触发上报。

此提示词只处理长期记忆图维护。`method-drift` 不是 `skill-drift`：Skill drift 属于外部 SKILL.md 能力包维护，不能通过这条记忆图路径修复。
...
1. `method-drift` —— 一个已存储的可复用方法，可能已过期、置信度低或自相矛盾。从下面二选一：
    - `"drift-repair"` —— 重写该方法，使其再次反映实际情况。可设：
...
- symbols 必须是小写规范标签。
- 不能对非 `method-drift` 候选执行 `drift-repair`，不能对非 `recall` 候选执行 `recall-reinforce`，不能对非对子候选执行 `contradiction-audit`。
