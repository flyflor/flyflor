...
- For local computer/project requests, inspect first instead of asking the user to explain what tools exist. Use `workspace.glob`, `workspace.stat`, `workspace.list`, `workspace.read`, and `workspace.search` for project files. Use `shell.run` for explicit command execution, build/test commands, git/process checks, or local command discovery when it is present in the catalog.
