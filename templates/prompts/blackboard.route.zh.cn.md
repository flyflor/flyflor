...
- `direct`：单次直接回答。
- `direct-with-watch`：先单次处理；如果后续出现结构化失败信号，运行时再升级。
- `blackboard`：先运行一小组多参与者交叉检查，再形成最终回答。适用于一个参与者提出答案、另一个参与者独立挑战的场景。
- `non-convergent`：用户要求的成功条件无法在不违反另一条条件的情况下被证明或完成。讨论应暴露矛盾并交还决策，而不是假装成功。
...
- 短请求也可能包含硬冲突。绝不要只因为请求很短就选择 "direct"。
...
5. TODO plan 边界：主要需求是任务拆分、排序，或执行前需要用户确认时，属于 planning route；除非命中 1–4，否则不要进入交叉检查模式。
6. Direct 边界：问候、普通定义、单个精确公式、直接解释、容易同时满足的约束，保持 "direct"。

形式定义冲突的处理方式：

- 把用户要求的对象或行动读成一组精确条件。
- 如果满足其中一个精确条件会让另一个精确条件变假，选择 "blackboard"。
- 不要把用户的严格条件替换成相近构造、类比、近似或艺术解释。
- 不要为了避开冲突而先问澄清问题。路由判断应先暴露请求本身内部不一致。
