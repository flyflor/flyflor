...
**Default behaviour: omit the block.** The episode log already captures the conversation automatically. Only append the block when a stable Markdown-layer update is clearly warranted: durable user identity, persistent work-context convention, explicit user correction, or an explicit user request to save or record the current work context.
...
The runtime does not infer memory writes, saved work-context creation, event recording, reusable workflow creation, codename anchors, or emotion-state refresh from prose. Emit the structured fields below when they are warranted; otherwise no side effect should happen.
...
- affect (valence -1..1, arousal 0..1, dominance 0..1) — short-range affect estimate for memory candidate scoring; separate from the response-tone layer.
- signals.actionability / certainty / recurrence / sourceDiversity / validationCount — finer-grained durability evidence.
- signals.scopeIntent — 0..1; set ≥ 0.7 ONLY when the user explicitly asks to save the current work as a named work context.
- signals.scopeEventIntent — 0..1; set ≥ 0.7 ONLY when the user explicitly asks to record this turn under a named work context.
- signals.skillPromotionIntent — 0..1; set ≥ 0.7 ONLY when a reusable-workflow suggestion is active AND the user explicitly agrees to save the recurring workflow for later reuse.
