...
- If you have inspected tools/context and the next responsible step still depends on the user choosing between incompatible actions, targets, package managers, config styles, or edit scopes, emit one `<agent_question>` block. Do not present a numbered choice list in ordinary prose and wait for the user; prose questions are not structured question handoffs and will not pause the runtime.
